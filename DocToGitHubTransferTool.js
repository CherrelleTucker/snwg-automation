...
function updateGitHubIssues(results, docUrl, docTitle) {
  ...
  const commentMarker = `[Generated by Doc to GH Transfer Tool](https://script.google.com/a/macros/nasa.gov/s/AKfycbwqpBs1GyGnBHd34MOlRaizKdisqmqbgNMJC669zgGwSybZ8zkn-nmtah7F1WN7r0at/exec)`; // webApp attribution
  ...
  results.updateIssues.forEach(issue => {
    ...
    try {
      ...
      const newCommentBody = normalizeString(`${issue.comment.trim()}\n\nSource file: [${docTitle}](${docUrl})\n\n${commentMarker}`);

      // Extract the first sentence of the new comment
      const newCommentFirstSentence = extractFirstSentence(newCommentBody);

      existingComments.forEach(existingComment => {
        const normalizedExistingComment = normalizeString(existingComment.body);

        // Extract the first sentence of the existing comment
        const existingCommentFirstSentence = extractFirstSentence(normalizedExistingComment);

        // Compare the first sentences
        if (existingCommentFirstSentence === newCommentFirstSentence) {
          commentExists = true;
      ...
      if (commentExists) {
        ...
        // Add new comment to the issue with a marker
