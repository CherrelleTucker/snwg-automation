...
function sendWeeklyAllDayEvents() {
  ...
  const allDayEvents = events.filter(event => event.isAllDayEvent() || isMultiDayEvent(event, monday, sunday));

  const message = buildEventMessage(allDayEvents, `This week's PI events:`, formatWeeklyEvent);
...
function isMultiDayEvent(event, weekStart, weekEnd) {
  ...
  return (startTime < weekEnd && endTime > weekStart) || (endTime.getDate() !== startTime.getDate());
}
...
function doPost(e) {
  try {
    ...
    if (commandText === '/picalendar') {
      ...
      const message = getDocsForRecentAndUpcomingEvents();
      return ContentService.createTextOutput(JSON.stringify({ text: message }))
        .setMimeType(ContentService.MimeType.JSON);
    } else if (commandText === '/picurrent') {
      const message = getCurrentMultiDayEvents();
      return ContentService.createTextOutput(JSON.stringify({ text: message }))
        .setMimeType(ContentService.MimeType.JSON);
    } else {
...
/**
 * Retrieves the descriptions for the next most recent past and future time-specific events.
 * 
 * @returns {string} The formatted message with event descriptions.
 */
function getDocsForRecentAndUpcomingEvents() {
  const now = new Date();
  const events = CalendarApp.getCalendarById(CALENDAR_ID).getEvents(new Date(now.getFullYear() - 1, now.getMonth(), now.getDate()), new Date(now.getFullYear() + 1, now.getMonth(), now.getDate()));
  ...
  const pastEventDescription = pastEvent ? cleanHtml(pastEvent.getDescription()) || "This file has not yet been created" : "This file has not yet been created";
  const futureEventDescription = futureEvent ? cleanHtml(futureEvent.getDescription()) || "This file has not yet been created" : "This file has not yet been created";
...
/**
 * Retrieves the titles of the multi-day events that span today.
 * 
 * @returns {string} The formatted message with event titles.
 */
function getCurrentMultiDayEvents() {
  const today = new Date();
  today.setHours(0, 0, 0, 0); // Start of the day
  const endOfDay = new Date(today);
  endOfDay.setHours(23, 59, 59, 999); // End of the day
  ...
  const events = CalendarApp.getCalendarById(CALENDAR_ID).getEvents(today, endOfDay);
  const multiDayEvents = events.filter(event => isMultiDayEvent(event, today, endOfDay));
