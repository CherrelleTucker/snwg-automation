...
// source file: test calendar id c_8798ebb71e4f29ffc300845dabe847152b8c92e2afd062e0e31242d7fce780cd@group.calendar.google.com
...
// There are three PI Planning events each PI. Each has a key phrase to match, in addition to the PI number. keyPhrase1 = "Welcome" keyPhrase2 = "Review" keyPhrase3 = "Final"
...
var CALENDAR_ID = "c_8798ebb71e4f29ffc300845dabe847152b8c92e2afd062e0e31242d7fce780cd@group.calendar.google.com";
...
function processPIEvent(event, folderId) {
  ...
  if (match && !description.includes(match[0]) && keyPhrase) {
    var files = getFilesFromFolder(folderId);
    var relevantFiles = files.filter(function(file) {
      return file.getName().includes(match[0]) && file.getName().includes(keyPhrase);
    });
    if (relevantFiles.length > 0) {
      event.setDescription(description + "\n" + formatFilesForDescription(relevantFiles));
    }
