// v2.0 Reorganized functions, corrected duplication, added not started status
...
function getActions() {
  ...
  for (let i = 0; i < firstTable.getNumRows(); i++) {
    const row = firstTable.getRow(i);
    const cell = row.getCell(1);

    const lines = cell.getText().split('\n');

    lines.forEach(line => {
      const startIndex = line.indexOf(actionsPhrase);

      if (startIndex !== -1) {
        const sentence = line.substring(startIndex + actionsPhrase.length);
